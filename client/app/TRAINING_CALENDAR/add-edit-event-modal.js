...
//---------------------------------------------------------
  ...
  timezones() {
    console.log('TIMEZONE: ', TimeZones.findOne());
    try {
      let tz    = TimeZones.findOne().timezones
...
Template.addEditEventModal.events({
  ...
  This training shall take place on ${moment(eventItem.start).format('M-D-Y')} at ${moment(eventItem.start).format('hh:mm a')} ${eventItem.timezone} and located at ${eventItem.location}.`;
          ...
          //in case we need to send email from company admin name. Replace admin@collectiveuniversity.com with adminEmail.
